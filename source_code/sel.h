...
Matrix createLocalK(int e,mesh &m){
    ...
    Matrix matrixA,matrixK,matrixC,matrixD,matrixH;
    ...
    productRealMatrix((J*lambda)/(9*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixC);

    //Preparando matrixD
    delta = m.getParameter(DELTA);
    productRealMatrix((-1)*(delta*Ae)/(8*D*D),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,BPrima,2,2,3),2,2,3),6,2,3),matrixD);

    //Preparando matrixH
    productRealMatrix(J/(6*D),productMatrixMatrix(BPrimat,productMatrixMatrix(Alphat,g_matrix_t,2,2,6),3,2,6),matrixH);

    //Colocando submatrices en K
    zeroes(K,9);
    ubicarSubMatriz(K,0,5,0,5,sumMatrix(matrixA,matrixK,6,6));
    ubicarSubMatriz(K,0,5,6,8,sumMatrix(matrixC,matrixD,6,3));
    ubicarSubMatriz(K,6,8,0,5,matrixH);
...
Vector createLocalb(int e,mesh &m){
    ...
    float f_x = m.getParameter(PSI_X);
    float f_y = m.getParameter(PSI_Y);
